...
`empyreand`是一个工具，使您能够与 Cosmos Hub 网络中的节点进行交互，无论您是否自己运行它。 让我们恰当的设置它。 要安装它，请按照[安装步骤](./ installation.md)进行安装。

### 配置 empyreand

设置`empyreand`的主要命令如下：
...
+ `cosmos`
  + 从通过`empyreand keys add`生成的账户私钥中产生
  + 用于接收资金
...
+ `cosmosvalconspub`
  + 在使用`empyreand init`创建节点时生成
  + 使用`empyreand tendermint show-validator`获得该值
...
在广播交易时，`empyreand`接受`--broadcast-mode`标识。 这个标识的值可以是`sync`（默认值）、`async`或`block`，其中`sync`使客户端返回 CheckTx 响应，`async`使客户端立即返回，而`block`使得 客户端等待 tx 被提交（或超时）。
...
```bash
empyreand tx bank send <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> \
  --dry-run
```
...
其中`[name]`是初始化`empyreand`时指定的键的名称。
...
可以通过完全命令生成主流的UNIX shell解释器（如`Bash`和`Zsh`）的`completion`命令，该命令可用于`empyreand`和`empyreand`。
