...
The current Cosmos Hub mainnet, `cosmoshub-4`, has been performing in place store migration upgrades as of the [Delta Upgrade](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-delta-upgrade.md) July 2021. The most recent upgrade was [Lambda](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-v9-Lambda-upgrade.md) March 2023. This type of upgrade preserves the same chain-id but state before the upgrade height is only accessible by corresponding versions of the binary:
- use `empyrean v5.0.x` (Delta) for queries of state between height `6,910,000` and `8,695,000`
- use `empyrean v6.0.x` (Vega) between `8,695,000` and `10,085,397`
- use `empyrean v7.0.x` (Theta) between `10,085,397` and `14,099,412`
- use `empyrean v8.0.x` (Rho) between `14,099,412` and `14,470,501`
- use `empyrean v9.0.x` (Lambda) between `14470501` and `15213800`
- use `empyrean v9.1.x` between `15213800` and `15816200`
- use `empyrean v10.0.x` from `15816200`
  
(ie. queries of state between height `6,910,000` and `8,695,000` should use `empyrean v5.0.x` (Delta), between `8,695,000` and `10,085,397` use `empyrean v6.0.x` (Vega), between `10,085,397` and `14,099,412` use `empyrean v7.0.x` (Theta),  between `14,099,412` and `14,470,501` use `empyrean v8.0.x` (Rho),  after `14,470,501` use `empyrean v9.0.x` (Lambda) to guarantee correctly encoded responses. The roadmap documentation contains a [history of upgrades](https://github.com/cosmos/empyrean/tree/main/docs/roadmap).). Visit the [migration section](https://github.com/cosmos/empyrean/tree/main/docs/migration) of the Hub's docs for more information on previous chain migrations.
...
The current Cosmos Hub mainnet `cosmoshub-4`. Visit the [migration section](https://github.com/cosmos/empyrean/tree/main/docs/migration) of the Hub's docs for more information on previous chain migrations.
...
- Ensure empyrean is properly installed. See the [installation guide](https://hub.cosmos.network/main/getting-started/installation.html) for a walk-through.
...
Make sure to walk through the basic setup and configuration. Operators will need to initialize `empyreand`, download the genesis file for `cosmoshub-4`, and set persistent peers and/or seeds for startup.

### Initialize Chain

Choose a custom moniker for the node and initialize. By default, the `init` command creates the `~/.empyrean` directory with subfolders `config` and `data`. In the `/config` directory, the most important files for configuration are `app.toml` and `config.toml`.

```bash
empyreand init <custom-moniker>
```
...
The `moniker` can be edited in the `~/.empyrean/config/config.toml` file:
...
Once the node is initialized, download the genesis file and move to the `/config` directory of the empyrean home directory.

```bash
wget https://raw.githubusercontent.com/cosmos/mainnet/master/genesis/genesis.cosmoshub-4.json.gz
gzip -d genesis.cosmoshub-4.json.gz
mv genesis.cosmoshub-4.json ~/.empyrean/config/genesis.json
```

### Seeds & Peers

Upon startup the node will need to connect to peers. If there are specific nodes a node operator is interested in setting as seeds or as persistent peers, this can be configured in `~/.empyrean/config/config.toml`
...
Node operators can optionally download the [Quicksync address book]( https://quicksync.io/addrbook.cosmos.json). Make sure to move this to `~/.empyrean/config/addrbook.json`.
...
A full-node keeps unconfirmed transactions in its mempool. In order to protect it from spam, it is better to set a `minimum-gas-prices` that the transaction must meet in order to be accepted in the node's mempool. This parameter can be set in  `~/.empyrean/config/app.toml`.
...
To set pruning, adjust the `pruning` parameter in the `~/.empyrean/config/app.toml` file.
...
Passing a flag when starting `empyrean` will always override settings in the `app.toml` file. To change the node's pruning setting to `everything` mode then pass the `---pruning everything` flag when running `empyreand start`.
...
By default, the REST API is disabled. To enable the REST API, edit the `~/.empyrean/config/app.toml` file, and set `enable` to `true` in the `[api]` section.
...
By default, gRPC is enabled on port `9090`. The `~/.empyrean/config/app.toml` file is where changes can be made in the gRPC section. To disable the gRPC endpoint, set `enable` to `false`. To change the port, use the `address` parameter.
...
It is possible to sync from previous versions of the Cosmos Hub. See the matrix below for the correct `empyrean` version. See the [mainnet archive](https://github.com/cosmos/mainnet) for historical genesis files.

| Chain Id      | empyrean Version  |
| -----------   | -------- |
| `cosmoshub-4` | `v4.2.1` |
| `cosmoshub-3` | `v2.0.x` |
| `cosmoshub-2` | `v1.0.x` |
...
Start empyrean to begin syncing with the `skip-invariants` flag. For more information on this see [Verify Mainnet](#Verify-Mainnet).
...
Start empyrean to begin state sync. It may take take some time for the node to acquire a snapshot, but the command and output should look similar to the following:
...
Once state sync successfully completes, the node will begin to process blocks normally. If state sync fails and the node operator encounters the following error:  `State sync failed err="state sync aborted"`, either try restarting `empyreand` or running `empyreand unsafe-reset-all` (make sure to backup any configuration and history before doing this).
::::::
...
While not advised, if a node operator needs to customize this feature, it can be configured in `~/.empyrean/config/app.toml`. The Cosmos Hub recommends setting this value to match `pruning-keep-every` in `config.toml`.
...
**See all [empyrean Releases](https://github.com/cosmos/empyrean/releases)**

The most up to date release of empyrean is [`V9.1.1`](https://github.com/cosmos/empyrean/releases/tag/v9.1.1). For those that want to use state sync or quicksync to get their node up to speed, starting with the most recent version of empyrean is sufficient.


To sync an archive or full node from scratch, it is important to note that you must start with [`V4.2.1`](https://github.com/cosmos/empyrean/releases/tag/v4.2.1) and proceed through two different upgrades Delta at block height `6,910,000`, Vega at block height `8,695,000`, Theta at block height `10,085,397`, Rho at block height `14099412` and Lambda at block height `14,470,501`.

The process is summarized below but make sure to follow the manual upgrade instructions for each release:

**[Delta Instructions](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-delta-upgrade.md#Upgrade-will-take-place-July-12,-2021)**
...
Make sure to save a backup of `~/.empyrean` in case rolling back is necessary.

Install empyrean [`V5.0.0`](https://github.com/cosmos/empyrean/releases/tag/v5.0.0) and restart the daemon.

**[Vega Instructions](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-vega-upgrade.md)**
...
Again, make sure to backup `~/.empyrean`

Install empyrean [`V6.0.0`](https://github.com/cosmos/empyrean/releases/tag/v6.0.0) and restart the daemon.

**[Theta Instructions](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-v7-Theta-upgrade.md)**
...
Install empyrean [`V7.0.0`](https://github.com/cosmos/empyrean/releases/tag/v7.0.0) and restart the daemon.


**[Rho Instructions](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-v8-Rho-upgrade.md)**
...
Install empyrean [`V8.0.0`](https://github.com/cosmos/empyrean/releases/tag/v8.0.0) and restart the daemon.

**[Lambda Instructions](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-v9-Lambda-upgrade.md)**
...
Install empyrean [`V9.0.0`](https://github.com/cosmos/empyrean/releases/tag/v9.0.0) and restart the daemon.
...
[Unit]
Description=empyrean Daemon
After=network-online.target

[Service]
User=$USER
ExecStart=$(which empyreand) start
Restart=always
RestartSec=3
LimitNOFILE=4096
...
```bash
Environment="DAEMON_HOME=$HOME/.empyrean"
Environment="DAEMON_NAME=empyreand"
Environment="DAEMON_ALLOW_DOWNLOAD_BINARIES=false"
Environment="DAEMON_RESTART_AFTER_UPGRADE=true"
```

After the `LimitNOFILE` line and replace `$(which empyreand)` with `$(which cosmovisor)`.
...
empyrean can dump the entire application state into a JSON file. This application state dump is useful for manual analysis and can also be used as the genesis file of a new network.
...
invariant broken:
    ...
    CRITICAL please submit the following transaction:
        empyreand tx crisis invariant-broken staking supply
