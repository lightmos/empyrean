...
echo "export DAEMON_NAME=empyreand" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.empyrean" >> ~/.profile
source ~/.profile
```
...
```bash
mkdir -p ~/.empyrean/cosmovisor/upgrades
mkdir -p ~/.empyrean/cosmovisor/genesis/bin/
cp $(which empyreand) ~/.empyrean/cosmovisor/genesis/bin/

# verify the setup. 
# It should return the same version as empyreand
cosmovisor version
```

Now `empyreand` can start by running
...
First, stop your instance of `empyreand`. Next, upgrade the software:
...
See the [testnet repo](https://github.com/cosmos/testnets) for details on which version is needed for which public testnet, and the [empyrean release page](https://github.com/cosmos/empyrean/releases) for details on each release.
...
```bash
cd $HOME/.empyrean/config
empyreand export --for-zero-height --height=<export-height> > new_genesis.json
```
...
::: warning
If you are running a **validator node** on the mainnet, always be careful when doing `empyreand unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
