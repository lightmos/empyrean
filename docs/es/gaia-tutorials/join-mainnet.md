...
::: aviso
**Necesitará [instalar empyrean](./installation.md) antes de avanzar más**
:::
...
```bash
empyreand init <your_custom_moniker>
```
...
Puede editar el apodo (`moniker`) después, en el archivo `~/.empyrean/config/config.toml`:
...
Puede editar el archivo `~/.empyrean/config/app.toml` para activar el mecanismo antispam y rechazar las transacciones entrantes con valores inferiores a los precios mínimos para el _gas_:
...
Busque el archivo `genesis.json` de la red principal en el directorio de configuración de `empyreand`.

```bash
mkdir -p $HOME/.empyrean/config
curl https://raw.githubusercontent.com/cosmos/launch/master/genesis.json > $HOME/.empyrean/config/genesis.json
```
...
Su nodo necesita saber cómo encontrar pares (_peers_). Necesita añadir nodos semilla en buen estado en `$HOME/.empyrean/config/config.toml`. El repositorio para el [`lanzamiento`](https://github.com/cosmos/launch) contiene enlaces a algunos nodos semilla.
...
Su nodo completo mantiene las transacciones no confirmadas en la _mempool_. Para protegerlo de ataques de spam, es mejor establecer un `minimum-gas-prices` que la transacción debe cumplir para ser aceptada en la _mempool_ de su nodo. Este parámetro puede ser establecido en el siguiente archivo `~/.empyrean/config/app.toml`.
...
Por defecto cada nodo está en modo `PruneSyncable`. Si desea cambiar su estrategia de reducción en su nodo, debe hacerlo cuando el nodo se ha iniciado. Por ejemplo, si desea cambiar su nodo al modo `PruneEverything` entonces puede pasar la opción `---pruning everything` cuando llame a `empyreand start`.
...
empyrean puede volcar todo el estado de la aplicación a un archivo JSON, que podría ser útil para el análisis manual y también puede ser usado como el archivo génesis para una nueva red.
...
```bash
empyreand start --inv-check-period [block-count]
```
