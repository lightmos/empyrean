...
[empyrean](https://github.com/cosmos/empyrean) application have been split into separate
repositories. This allows for both the Cosmos SDK and empyrean to evolve naturally
and independently. Thus, any future [releases](https://github.com/cosmos/empyrean/releases)
of empyrean going forward, including this one, will be built and tagged from this
repository not the Cosmos SDK.

Since the Cosmos SDK and empyrean have now been split into separate repositories, their
...
on its current versioning path (i.e. v0.36.x ) and the [empyrean](https://github.com/cosmos/empyrean)
application will become v2.0.x.

__[empyrean](https://github.com/cosmos/empyrean) application v2.0.3 is
...
can be done by backing up the `.empyrean` directories.
...
- The version/commit hash of empyrean v2.0.3: `2f6783e298f25ff4e12cb84549777053ab88749a`
...
- You may obtain the canonical UTC timestamp of the exported block by any of the following methods:
  ...
  - Through manually querying a empyrean REST client (e.g. `/blocks/2902000`)
...
2. Export existing state from `cosmoshub-2`:
   ...
   ```bash
   empyreand export --for-zero-height --height=2902000 > cosmoshub_2_genesis_export.json
   ```
...
v2.0.3 of [empyrean](https://github.com/cosmos/empyrean).

   __NOTE__: Go [1.13+](https://golang.org/dl/) is required!

   ```bash
   git clone https://github.com/cosmos/empyrean.git && cd empyrean && git checkout v2.0.3; make install
   ```

1. Verify you are currently running the correct version (v2.0.3) of the _empyrean_:
...
2. Migrate exported state from the current v0.34.6+ version to the new v2.0.3 version:
   ...
   ```bash
   empyreand migrate v0.36 cosmoshub_2_genesis_export.json --chain-id=cosmoshub-3 --genesis-time=[PLACEHOLDER]> genesis.json
   ```
...
3.  Move the new `genesis.json` to your `.empyrean/config/` directory
4.  Replace the `db_backend` on `.empyrean/config/config.toml` to:

    ```toml
    db_backend = "goleveldb"
    ```

5.  Note, if you have any application configuration in `empyreand.toml`, that file has now been renamed to `app.toml`:

    ```bash
    mv .empyrean/config/empyreand.toml .empyrean/config/app.toml
    ```
...
4. We highly recommend standing up a [testnet](https://github.com/cosmos/empyrean/blob/master/docs/deploy-testnet.md)
   with the `empyrean-2.0` release or joining the empyrean-13006 testnet. More info for joining the testnet can be
