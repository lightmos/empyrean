...
This document describes the steps for validator and full node operators for the successful execution of the [Delta Upgrade](https://github.com/cosmos/empyrean/blob/main/docs/roadmap/cosmos-hub-roadmap-2.0.md#Delta-Upgrade), which adds the __Gravity DEX__ to the Cosmos Hub.
...
- [Preparing for the upgrade](#preparing-for-the-upgrade)
  ...
  - [Current runtime, cosmoshub-4 (pre-Delta upgrade) is running empyrean v4.2.1](#current-runtime-cosmoshub-4-pre-delta-upgrade-is-running-empyrean-v421)
  - [Target runtime, cosmoshub-4 (post-Delta upgrade) will run empyrean v5.0.0](#target-runtime-cosmoshub-4-post-delta-upgrade-will-run-empyrean-v500)
...
### Current runtime, cosmoshub-4 (pre-Delta upgrade) is running empyrean v4.2.1

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [empyrean v4.2.1](https://github.com/cosmos/empyrean/releases/tag/v4.2.1). We anticipate that operators who are running earlier versions of empyrean, e.g., v4.2.x, will be able to upgrade successfully; however, this is untested and it is up to operators to ensure that their systems are capable of performing the upgrade.

### Target runtime, cosmoshub-4 (post-Delta upgrade) will run empyrean v5.0.0

The Comsos Hub mainnet network, `cosmoshub-4`, will run [empyrean v5.0.0](https://github.com/cosmos/empyrean/releases/tag/v5.0.0). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
1. Prior to the upgrade, operators _MUST_ be running empyrean v4.2.1.
2. At the upgrade block height of [6910000](#Upgrade-will-take-place-July-12,-2021), the empyrean software will panic with a message similar to the below:
...
```bash
cp -r ~/.empyrean ./empyrean_backup
```

Note: use the home directory relevant to your node's empyrean configuration (if different from `~/.empyrean`).

4. Replace the empyrean v4.2.1 binary with the empyrean v5.0.0 binary
5. Start the empyrean v5.0.0 binary using the following command (also applying any additional flags and parameters to the binary needed by the operator, e.g., `--home $HOME`):
...
> empyreand start --unsafe-skip-upgrade 6910000
...
- Use a post-delta upgrade state snapshot, such as one provided by [quicksync](https://cosmos.quicksync.io/) and start a node using the empyrean v5.0.0 binary.
- If not using a snapshot, or using a pre-delta upgrade snapshot, sync with the network using the empyrean v4.2.1 binary until the upgrade height and panic, then switch the empyrean binary for v5.0.0.
