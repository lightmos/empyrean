...
- Upgrading Golang to [Golang 1.20.x](https://go.dev/blog/go1.20), making it mandatory to build empyrean with **Golang v1.20.x**

❗The **preferred binary** for **Mainnet release** is [v10.0.1](https://github.com/cosmos/empyrean/releases/tag/v10.0.1), as that version includes a fix for the [IBC Huckleberry fix](https://forum.cosmos.network/t/ibc-security-advisory-huckleberry/10731). v10.0.0 does **NOT** include this fix.
...
- [Preparing for the upgrade](#preparing-for-the-upgrade)
    ...
    - [Backups](#backups)
    - [Testing](#testing)
    - [Current runtime, cosmoshub-4 (pre-v10 upgrade) is running empyrean v9.1.1](#current-runtime-cosmoshub-4-pre-v10-upgrade-is-running-empyrean-v911)
    - [Target runtime, cosmoshub-4 (post-v10 upgrade) will run empyrean v10.0.1](#target-runtime-cosmoshub-4-post-v10-upgrade-will-run-empyrean-v1000)
- [Upgrade steps](#upgrade-steps)
    ...
    - [Method II: Upgrade using Cosmovisor](#method-ii-upgrade-using-cosmovisor)
        ...
        - [Auto-Downloading the empyrean binary](#auto-downloading-the-empyrean-binary)
...
### Current runtime, cosmoshub-4 (pre-v10 upgrade) is running empyrean v9.1.1

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [empyrean v9.1.1](https://github.com/cosmos/empyrean/releases/v9.1.1). We anticipate that operators who are running on v9.1.1, will be able to upgrade successfully. Validators are expected to ensure that their systems are up to date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.20`.

### Target runtime, cosmoshub-4 (post-v10 upgrade) will run empyrean v10.0.1

The Cosmos Hub mainnet network, `cosmoshub-4`, will run [empyrean v10.0.1](https://github.com/cosmos/empyrean/releases/tag/v10.0.1). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
Make sure empyrean v10.0.1 is installed by either downloading a [compatible binary](https://github.com/cosmos/empyrean/releases/tag/v10.0.1), or building from source. Building from source requires **Golang 1.20**.

Run empyrean v9.1.1 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v10" NEEDED at height: 15816200: upgrade to v10 and applying upgrade "v10" at height:15816200
```

Stop the node, and switch the binary to empyrean v10.0.1 and re-start by `empyreand start`.
...
create a Cosmovisor folder inside `$empyrean_HOME` and move empyrean v9.1.1 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
build empyrean v10.0.1, and move empyreand v10.0.1 to `$empyrean_HOME/cosmovisor/upgrades/v10/bin`

```shell
mkdir -p  $empyrean_HOME/cosmovisor/upgrades/v10/bin
cp $(which empyreand) $empyrean_HOME/cosmovisor/upgrades/v10/bin
```
...
### Auto-Downloading the empyrean binary
...
create a cosmovisor folder inside empyrean home and move empyreand v9.1.1 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$empyrean_HOME/cosmovisor/upgrades/v10/bin` and download the empyrean v10.0.1 binary to this folder according to links in the `--info` field of the upgrade proposal.
...
> empyreand start --unsafe-skip-upgrade 15816200
