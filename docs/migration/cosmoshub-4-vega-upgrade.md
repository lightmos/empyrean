...
This document describes the steps for validator and full node operators for the successful execution of the [Vega Upgrade](https://github.com/cosmos/empyrean/blob/main/docs/roadmap/cosmos-hub-roadmap-2.0.md#vega-upgrade-expected-q4-2021), which contains the following main new features:
...
- [Preparing for the upgrade](#preparing-for-the-upgrade)
  ...
  - [Backups](#backups)
  - [Testing](#testing)
  - [Current runtime, cosmoshub-4 (pre-Vega upgrade) is running empyrean v5.0.0](#current-runtime-cosmoshub-4-pre-vega-upgrade-is-running-empyrean-v500)
  - [Target runtime, cosmoshub-4 (post-Vega upgrade) will run empyrean v6.0.4](#target-runtime-cosmoshub-4-post-vega-upgrade-will-run-empyrean-v604)
- [Vega upgrade steps](#vega-upgrade-steps)
  - [Method I: manual upgrade](#method-i-manual-upgrade)
  - - [Method II: upgrade using Cosmovisor by manually preparing the empyrean v6.0.4 binary](#method-ii-upgrade-using-cosmovisor-by-manually-preparing-the-empyrean-v604-binary)
  - [Method III: upgrade using Cosmovisor by auto-downloading the empyrean v6.0.4 binary (not recommended!)](#method-iii-upgrade-using-cosmovisor-by-auto-downloading-the-empyrean-v604-binary-not-recommended)
...
If you use Cosmovisor to upgrade, by default, Cosmovisor will backup your data upon upgrade. See below [upgrade by cosmovisor](#method-ii-upgrade-using-cosmovisor-by-manually-preparing-the-empyrean-v600-binary) section.
...
### Current runtime, cosmoshub-4 (pre-Vega upgrade) is running empyrean v5.0.0

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [empyrean v5.0.0](https://github.com/cosmos/empyrean/releases/tag/v5.0.0). We anticipate that operators who are running on v5.0.x, will be able to upgrade successfully; however, this is untested and it is up to operators to ensure that their systems are capable of performing the upgrade.

### Target runtime, cosmoshub-4 (post-Vega upgrade) will run empyrean v6.0.4

The Comsos Hub mainnet network, `cosmoshub-4`, will run [empyrean v6.0.4](https://github.com/cosmos/empyrean/releases/tag/v6.0.4). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
Run empyrean v5.0.x till upgrade height, the node will panic:
...
Stop the node, and install empyrean v6.0.4 and re-start by `empyreand start`.
...
### Method II: upgrade using Cosmovisor by manually preparing the empyrean v6.0.4 binary
...
create a Cosmovisor folder inside `$empyrean_HOME` and move empyrean v5.0.0 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
build empyrean v6.0.4, and move empyreand v6.0.4 to `$empyrean_HOME/cosmovisor/upgrades/Vega/bin`

```shell
mkdir -p  $empyrean_HOME/cosmovisor/upgrades/Vega/bin
cp $(which empyreand) $empyrean_HOME/cosmovisor/upgrades/Vega/bin
```
...
### Method III: upgrade using Cosmovisor by auto-downloading the empyrean v6.0.4 binary (not recommended!)
...
create a cosmovisor folder inside empyrean home and move empyreand v5.0.0 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$empyrean_HOME/cosmovisor/upgrades/Vega/bin` and download empyrean v6.0.4 binary to this folder according to links in the `--info` field of the upgrade proposal 59.
...
- For the Vega upgrade, empyrean will upgrade its dependency on Cosmos SDK v0.42 to Cosmos SDK v0.44, this will require [Cosmovisor v0.1](https://github.com/cosmos/cosmos-sdk/releases/tag/cosmovisor%2Fv0.1.0). Later versions of Cosmovisor do not support Cosmos SDK v0.42 or earlier if the auto-download option is enabled.
- By using Cosmovisor v0.1 you might experience a [node hanging issue](https://github.com/cosmos/cosmos-sdk/issues/9875) when querying a result with a large output size. For example, `empyreand q gov proposals` will hang the node being queried, this issue will not appear for Cosmovisor versions newer than v0.1.
...
> empyreand start --unsafe-skip-upgrade 8695000
...
[Delta upgrade instruction](https://github.com/cosmos/empyrean/blob/main/docs/migration/cosmoshub-4-delta-upgrade.md)
