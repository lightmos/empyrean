...
This document describes the steps for validator and full node operators for the successful execution of the [v8-Rho Upgrade](https://github.com/cosmos/empyrean/blob/main/docs/roadmap/cosmos-hub-roadmap-2.0.md#v8-rho-upgrade-expected-q1-2023), which contains the following main new features/improvement:
...
- [globalfee](https://github.com/cosmos/empyrean/tree/main/x/globalfee) module. See [globalfee docs](https://github.com/cosmos/empyrean/blob/main/docs/modules/globalfee.md) for more details.
- [#1845](https://github.com/cosmos/empyrean/pull/1845) Add bech32-convert command to empyreand.
- [Add new fee decorator](https://github.com/cosmos/empyrean/pull/1961) to change `MaxBypassMinFeeMsgGasUsage` so importers of x/globalfee can change `MaxGas`.
- [#1870](https://github.com/cosmos/empyrean/issues/1870) Fix bank denom metadata in migration. See [#1892](https://github.com/cosmos/empyrean/pull/1892) for more details.
- [#1976](https://github.com/cosmos/empyrean/pull/1976) Fix Quicksilver ICA exploit in migration. See [the bug fix forum post](https://forum.cosmos.network/t/upcoming-interchain-accounts-bugfix-release/8911) for more details.
- [E2E tests](https://github.com/cosmos/empyrean/tree/main/tests/e2e). The tests cover transactions/queries tests of different modules, including Bank, Distribution, Encode, Evidence, FeeGrant, Global Fee, Gov, IBC, packet forwarding middleware, Slashing, Staking, and Vesting module.
- [#1941](https://github.com/cosmos/empyrean/pull/1941) Fix packet forward configuration for e2e tests.
- Use empyreand to swap out [Ignite](https://github.com/ignite/cli) in [liveness tests](https://github.com/cosmos/empyrean/blob/main/.github/workflows/test.yml).
...
- [Cosmos Hub 4, v8-Rho Upgrade, Instructions](#cosmos-hub-4-v8-rho-upgrade-instructions)
  ...
  - [Preparing for the upgrade](#preparing-for-the-upgrade)
    ...
    - [Backups](#backups)
    - [Testing](#testing)
    - [Current runtime, cosmoshub-4 (pre-v8-Rho upgrade) is running empyrean v7.0.x](#current-runtime-cosmoshub-4-pre-v7-theta-upgrade-is-running-empyrean-v60x)
    - [Target runtime, cosmoshub-4 (post-v8-Rho upgrade) will run empyrean v8.0.0](#target-runtime-cosmoshub-4-post-v8-rho-upgrade-will-run-empyrean-v800)
  - [v8-Rho upgrade steps](#v8-Rho-upgrade-steps)
    - [Method I: Manual Upgrade](#method-i-manual-upgrade)
    - [Method II: Upgrade using Cosmovisor](#method-ii-upgrade-using-cosmovisor)
      - [Manually preparing the binary](#manually-preparing-the-empyrean-v800-binary)
        - [Preparation](#preparation)
        - [Expected upgrade result](#expected-upgrade-result)
      - [Auto-Downloading the empyrean v8.0.0 binary (not recommended!)](#auto-downloading-the-empyrean-v800-binary-not-recommended)
...
### Current runtime, cosmoshub-4 (pre-v8-Rho upgrade) is running empyrean v7.1.1

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [empyrean v7.1.1](https://github.com/cosmos/empyrean/releases/v7.1.1). We anticipate that operators who are running on v7.1.1, will be able to upgrade successfully; however, this is untested and it is up to operators to ensure that their systems are capable of performing the upgrade.

### Target runtime, cosmoshub-4 (post-v8-Rho upgrade) will run empyrean v8.0.0

The Cosmos Hub mainnet network, `cosmoshub-4`, will run [empyrean v8.0.0](https://github.com/cosmos/empyrean/releases/tag/v8.0.0). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
Run empyrean v7.1.1 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v8-Rho" NEEDED at height: 14099412: upgrade to v7-Theta and applying upgrade "v8-Rho" at height:14099412
```

Stop the node, and install empyrean v8.0.0 and re-start by `empyreand start`.
...
> 
> **For Example:** <br>
> **Cosmovisor =< `1.1.0`: `/upgrades/v8-Rho/bin/empyreand`**<br>
> **Cosmovisor >= `1.2.0`: `/upgrades/v8-rho/bin/empyreand`**<br>
...
### _Manually preparing the empyrean v8.0.0 binary_
...
create a Cosmovisor folder inside `$empyrean_HOME` and move empyrean v7.1.1 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
build empyrean v8.0.0, and move empyreand v8.0.0 to `$empyrean_HOME/cosmovisor/upgrades/v8-rho/bin`

```shell
mkdir -p  $empyrean_HOME/cosmovisor/upgrades/v8-rho/bin
cp $(which empyreand) $empyrean_HOME/cosmovisor/upgrades/v8-rho/bin
```
...
When the upgrade block height is reached, empyrean will panic and stop:
...
### _Auto-Downloading the empyrean v8.0.0 binary (not recommended!)_
#### Preparation
...
create a cosmovisor folder inside empyrean home and move empyreand v7.1.1 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$empyrean_HOME/cosmovisor/upgrades/v8-rho/bin` and download the empyrean v8.0.0 binary to this folder according to links in the `--info` field of the upgrade proposal 97.
...
> empyreand start --unsafe-skip-upgrade 14099412
