...
This document describes the steps for validator and full node operators for the successful execution of the [v9-Lambda Upgrade](https://github.com/cosmos/empyrean/blob/main/docs/roadmap/cosmos-hub-roadmap-2.0.md#v9-lambda-upgrade-expected-q1-2023), which contains the following main new features/improvement:
...
- [E2E ccv tests](https://github.com/cosmos/empyrean/blob/main/tests/e2e/e2e_gov_test.go#L138). Tests covering new functionality introduced by the provider module to add and remove a consumer chain via governance proposal.
- [integration ccv tests](https://github.com/cosmos/empyrean/blob/main/tests/ics/interchain_security_test.go). Imports Interchain-Security's `TestCCVTestSuite` and implements empyrean as the provider chain.
...
- [Cosmos Hub 4, v9-Lambda Upgrade, Instructions](#cosmos-hub-4-v9-lambda-upgrade-instructions)
    ...
    - [Preparing for the upgrade](#preparing-for-the-upgrade)
        ...
        - [Backups](#backups)
        - [Testing](#testing)
        - [Current runtime, cosmoshub-4 (pre-v9-Lambda upgrade) is running empyrean v8.0.1](#current-runtime-cosmoshub-4-pre-v9-lambda-upgrade-is-running-empyrean-v801)
        - [Target runtime, cosmoshub-4 (post-v9-Lambda upgrade) will run empyrean v9.0.0](#target-runtime-cosmoshub-4-post-v9-lambda-upgrade-will-run-empyrean-v900)
    - [v9-Lambda upgrade steps](#v9-Lambda-upgrade-steps)
        ...
        - [Method II: Upgrade using Cosmovisor](#method-ii-upgrade-using-cosmovisor)
            - [Manually preparing the binary](#manually-preparing-the-empyrean-v900-binary)
                - [Preparation](#preparation)
                - [Expected upgrade result](#expected-upgrade-result)
            - [Auto-Downloading the empyrean v9.0.0 binary (not recommended!)](#auto-downloading-the-empyrean-v900-binary-not-recommended)
...
### Current runtime, cosmoshub-4 (pre-v9-Lambda upgrade) is running empyrean v8.0.1

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [empyrean v8.0.1](https://github.com/cosmos/empyrean/releases/v8.0.1). We anticipate that operators who are running on v8.0.1, will be able to upgrade successfully. Validators are expected to ensure that their systems are up to date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.18`.

### Target runtime, cosmoshub-4 (post-v9-Lambda upgrade) will run empyrean v9.0.0

The Cosmos Hub mainnet network, `cosmoshub-4`, will run [empyrean v9.0.0](https://github.com/cosmos/empyrean/releases/tag/v9.0.0). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
Make sure empyrean v9.0.0 is installed by either downloading a [compatable binary](https://github.com/cosmos/empyrean/releases/tag/v9.0.0), or building from source. Building from source requires go 1.18.

Run empyrean v8.0.1 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v9-Lambda" NEEDED at height: 14470501: upgrade to v9-Lambda and applying upgrade "v9-Lambda" at height:14470501
```

Stop the node, and switch the binary to empyrean v9.0.0 and re-start by `empyreand start`.
...
> **For Example:** <br>
> **Cosmovisor =< `1.1.0`: `/upgrades/v9-Lambda/bin/empyreand`**<br>
> **Cosmovisor >= `1.2.0`: `/upgrades/v9-lambda/bin/empyreand`**<br>
...
### _Manually preparing the empyrean v9.0.0 binary_
...
create a Cosmovisor folder inside `$empyrean_HOME` and move empyrean v8.0.1 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
build empyrean v9.0.0, and move empyreand v9.0.0 to `$empyrean_HOME/cosmovisor/upgrades/v9-lambda/bin`

```shell
mkdir -p  $empyrean_HOME/cosmovisor/upgrades/v9-lambda/bin
cp $(which empyreand) $empyrean_HOME/cosmovisor/upgrades/v9-lambda/bin
```
...
### _Auto-Downloading the empyrean v9.0.0 binary (not recommended!)_
#### Preparation
...
create a cosmovisor folder inside empyrean home and move empyreand v8.0.1 into `$empyrean_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$empyrean_HOME/cosmovisor/upgrades/v9-lambda/bin` and download the empyrean v9.0.0 binary to this folder according to links in the `--info` field of the upgrade proposal 97.
...
> empyreand start --unsafe-skip-upgrade 14470501
