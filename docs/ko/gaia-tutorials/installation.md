...
이 가이드는 `empyreand`와 `empyreand`를 엔트리포인트를 시스템에 설치하는 방법을 설명합니다. `empyreand`와 `empyreand`가 설치된 서버를 통해 [풀노드](./empyrean-tutorials/join-testnet.md#run-a-full-node) 또는 [밸리데이터로](./validators/validator-setup.md)써 최신 테스트넷에 참가하실 수 있습니다.
...
다음은 최신 empyrean 버전을 설치하는 것입니다. 필요에 따라 `git checkout`을 통해 올바른 [릴리즈 버전](https://github.com/cosmos/empyrean/releases)이 설치되어있는지 확인하세요.

```bash
git clone -b <latest-release-tag> https://github.com/cosmos/empyrean
...
위 절차를 따라하시면 `empyreand`와 `empyreand` 바이너리가 설치될 것입니다. 설치가 잘 되어있는지 확인하십시오:
...
`empyreand` 명령어는 다음과 비슷한 아웃풋을 내보냅니다:

```bash
name: empyrean
server_name: empyreand
client_name: empyreand
version: 1.0.0
...
축하합니다! 이제 [메인넷](./empyrean-tutorials/join-mainnet.md)에 참가하거나[퍼블릭 테스트넷](./join-testnet.md)에 참가하시거나 [자체 테스트넷](./empyrean-tutorials/deploy-testnet.md)을 운영하실 수 있습니다.
