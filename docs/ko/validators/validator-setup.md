...
```bash
empyreand tx staking create-validator \
  --amount=5000000uatom \
  --pubkey=$(empyreand tendermint show-validator) \
...
__참고__: `consensus_pubkey` 값이 지정되지 않은 경우, 기본적으로 `empyreand tendermint show-validator` 의 값으로 설정됩니다. `key_name`은 트랙잭션을 서명할때 이용되는 프라이빗키의 명칭입니다.
...
__참고__: 이 명령어는 제네시스에서의 처리를 위해 `gentx`를 `~/.empyrean/config/gentx`에 저장합니다.

::: tip
명령어 플래그에 대한 정보는 `empyreand gentx --help`를 사용에 확인하십시오.
:::
...
우선 `genesis.json`파일을 `empyreand`의 config 디렉토리로 가져옵니다.

Fetch the `genesis.json` file into `empyreand`'s config directory.

```bash
mkdir -p $HOME/.empyrean/config
curl https://raw.githubusercontent.com/cosmos/testnets/master/latest/genesis.json > $HOME/.empyrean/config/genesis.json
```
...
모든 제네시스 트랜잭션을 받으시고 `~/.empyrean/config/gentx`에 저장하셨다면 다음 명령어를 실행하십시오:

```bash
empyreand collect-gentxs
```
...
```bash
empyreand tx staking edit-validator
  --moniker="choose a moniker" \
...
```bash
empyreand query staking validator <account_cosmos>
```
...
```bash
empyreand query tendermint-validator-set | grep "$(empyreand tendermint show-validator)"
```

코스모스 테스트넷의 경우 코스모스 [익스플로러](https://explorecosmos.network/validators)를 통해서 밸리데이터가 운영되고 있는지 확인하실 수 있습니다. `~/.empyrean/config/priv_validator.json` 파일의 `bech32` 인코딩이된 `address` 항목을 참고하세요.
...
밸리데이터가 자동 언본딩 되었습니다. `empyrean-8000`의 경우, 100개 블록 중 50개의 블록에 투표하지 않을 경우 언본딩 됩니다. 블록은 대략 ~2초 마다 생성되기 때문에 ~100초 이상 비활성화 상태를 유지하는 밸리데이터는 언본딩 될 수 있습니다. 가장 흔한 이유는 `empyreand` 프로세스가 멈춘 경우입니다.

보팅 파워를 다시 밸리데이터에게 되돌리기 위해서, 우선 `empyreand`가 실행되고 있는지 확인하십시오. 만약 실행되고 있지 않은 경우 다음 명령어를 실행하십시오:

```bash
empyreand start
```

당신의 풀노드가 최신 블록높이에 싱크될때까지 기다리십시오. 이후, 다음 명령어를 실행하십시오. 참고로 `<cosmos>` 항목은 밸리데이터 계정의 주소이며, `<name>`은 밸리데이터 계정의 이름입니다. 해당 정보는 `empyreand keys list` 명령어를 통해 확인하실 수 있습니다.

```bash
empyreand tx slashing unjail <cosmos> --chain-id=<chain_id> --from=<from>
```

::: danger 경고
`empyreand`가 싱크되지 않은 상태에서 `unjail` 명령을 실행하실 경우, 검증인이 아직 '구속' 상태라는 메시지를 받게 됩니다.
:::
...
### 문제 #2: `too many open files`때문에 `empyreand`가 강제 종료됩니다

리눅스가 각 프로세스당 열 수 있는는 파일 수는 최대 `1024`개입니다. `empyreand`는 1024개 이상의 열게될 수 있음으로 프로세스가 중단될 수 있습니다. 가장 간편한 해결책은 `ulimit -n 4096` (열 수 있는 최대 파일 수)명령어를 입력하고 프로세스를 `empyreand start`로 재시작하는 것입니다. 만약 `systemd` 또는 다른 프로세스 매니저로 `empyreand`를 실행하신다면 해당 레벨에서 몇가지 설정을 해야합니다. 문제 해결 샘플 `systemd` 파일은 다음과 같습니다:

```toml
# /etc/systemd/system/empyreand.service
[Unit]
Description=Cosmos empyrean Node
...
ExecStart=/home/ubuntu/go/bin/empyreand start
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
