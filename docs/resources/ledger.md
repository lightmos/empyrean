...
Installing the `Cosmos` application on your ledger device is required before you can use either [Keplr](#keplr-+-ledger-nano) or [`empyreand`](#empyrean-cli-+-ledger-nano). To do so, you need to:
...
The tool used to generate addresses and transactions on the Cosmos Hub network is `empyreand`. Here is how to get started. If using a CLI tool is unfamiliar to you, scroll down and follow instructions for using the Keplr wallet instead.

### Before you Begin

- [Install Golang](https://golang.org/doc/install)
- [Install empyrean](../getting-started/installation.md)

Verify that empyreand is installed correctly with the following command
...
- Create an account in empyreand from your ledger key.

::: tip
Be sure to change the _keyName_ parameter to be a meaningful name. The `ledger` flag tells `empyreand` to use your Ledger to seed the account.
:::

```bash
empyreand keys add <keyName> --ledger
...
```bash
empyreand keys add <secondKeyName> --ledger --account <i>
```
...
Next, you need to configure empyreand with the URL of a Cosmos full node and the appropriate `chain_id`. In this example we connect to the public load balanced full node operated by Chorus One on the `cosmoshub-2` chain. But you can point your `empyreand` to any Cosmos full node. Be sure that the `chain-id` is set to the same chain as the full node.

```bash
empyreand config node https://cosmos.chorus.one:26657
empyreand config chain_id cosmoshub-2
```
...
You are now ready to start signing and sending transactions. Send a transaction with empyreand using the `tx send` command.

``` bash
empyreand tx bank send --help # to see all available options.
```
...
Use the `keyName` you set for your Ledger key and empyrean will connect with the Cosmos Ledger app to then sign your transaction.

```bash
empyreand tx bank send <keyName> <destinationAddress> <amount><denomination>
```
...
Not sure what `empyreand` can do? Simply run the command without arguments to output documentation for the commands in supports.

::: tip
The `empyreand` help commands are nested. So `$ empyreand` will output docs for the top level commands (status, config, query, and tx). You can access documentation for sub commands with further help commands.

For example, to print the `query` commands:

```bash
empyreand query --help
```
...
- `chain-id`: The chain to which you are broadcasting the tx, such as the `empyrean-13003` testnet or `cosmoshub-2`: mainnet.
