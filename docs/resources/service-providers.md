...
- [Command-Line Interface](#command-line-interface)
  ...
  - [Remote Access to empyreand](#remote-access-to-empyreand)
...
The command-line interface (CLI) is the most powerful tool to access the Cosmos Hub and use empyrean.
To use the CLI, you must install the latest version of `empyrean` on your machine.

Compare your version with the [latest release version](https://github.com/cosmos/empyrean/releases)
...
All available CLI commands are shown when you run the `empyreand` command:
...
Flags:
  -h, --help                help for empyreand
      --home string         directory for config and data (default "/Users/tobias/.empyrean")
      --log_format string   The logging format (json|plain) (default "plain")
      --log_level string    The logging level (trace|debug|info|warn|error|fatal|panic) (default "info")
      --trace               print out full stack trace on errors

Use "empyreand [command] --help" for more information about a command.
```
...
Usage:
  empyreand query [flags]
  empyreand query [command]
...
Use "empyreand query [command] --help" for more information about a command.
```

### Remote Access to empyreand

When choosing to remote access a Full Node and empyreand, you need a Full Node running and empyrean installed on your local machine.

`empyreand` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not.

To set up `empyreand` on a local machine and connect to an existing full node, use the following command:
...
```bash
empyreand config chain-id cosmoshub-4
```
...
The default key is `secp256k1 elliptic curve`. Use the `empyreand keys` command to list the keys and generate a new key.

```bash
empyreand keys add <your_key_name>
```
...
```bash
empyreand keys add <your_key_name> --recover
```
...
```bash
empyreand query account <YOUR_ADDRESS>
```
...
```bash
empyreand query bank balances <YOUR_ADDRESS>
```
