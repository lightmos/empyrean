...
There are ways to query the current settings for each module's parameter(s). Some can be queried with the command line program [`empyreand`](../../getting-started/installation.md).

You can begin by using the command `empyrean q [module] -h` to get help about the subcommands for the module you want to query. For example, `empyreand q staking params --chain-id <chain-id> --node <node-id>` returns the settings of relevant parameters:
