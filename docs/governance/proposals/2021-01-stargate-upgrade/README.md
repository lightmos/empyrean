...
------------
If passed, this governance proposal would commit the Cosmos Hub to halting the `cosmoshub-3` at 06:00 UTC on Jan 28th exporting the state and starting `cosmoshub-4` based on empyrean 3.0.
...
This section is with the current `empyrean 2.0.*` implementation.
  ...
  1. Validators should set their `empyrean` with a halt time of `1611813600`ie. 06:00 UTC on Jan 28th in Unix time.
...
This section is with the upgraded `empyrean 3.0.*` implemenataion.
...
A testnet with the final version of empyrean 3.0 will be running to assist with integrations.
...
1. A critical vulnerability may be found in the software. If the development teams change their recommended version of empyrean, the validator set should implicitly abandon this upgrade procedure. A future proposal will be made to the Hub to upgrade to the new target commit.
