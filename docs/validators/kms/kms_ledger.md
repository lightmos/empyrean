...
chain_id = "empyrean-11001"
reconnect = true # true is the default
secret_key = "~/.tmkms/secret_connection.key"

[[providers.ledgertm]]
chain_ids = ["empyrean-11001"]
```

- Edit `addr` to point to your `empyreand` instance.
- Adjust `chain-id` to match your `.empyrean/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `empyreand`.
...
The KMS may complain that it cannot connect to empyreand. That is fine, we will fix it in the next section.
...
## empyrean configuration

You need to enable KMS access by editing `.empyrean/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `empyreand`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell empyreand the key that we've got in the previous section.

```bash
empyreand gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `empyreand`. You should see that the KMS connects and receives a signature request.
