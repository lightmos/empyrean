...
> **Note**: Make sure the [empyrean CLI is installed](./installation.md).
...
```bash
# Build empyreand binary and initialize chain
cd $HOME
git clone -b v10.0.0 https://github.com/cosmos/empyrean --depth=1
cd empyreand
make install
empyreand init CUSTOM_MONIKER --chain-id cosmoshub-4

#Set minimum gas price & peers
sed -i'' 's/minimum-gas-prices = ""/minimum-gas-prices = "0.0025uatom"/' $HOME/.empyrean/config/app.toml
sed -i'' 's/persistent_peers = ""/persistent_peers = '"\"$(curl -s https://raw.githubusercontent.com/cosmos/chain-registry/master/cosmoshub/chain.json | jq -r '[foreach .peers.seeds[] as $item (""; "\($item.id)@\($item.address)")] | join(",")')\""'/' $HOME/.empyrean/config/config.toml

# Configure State sync
sed -i'' 's/enable = false/enable = true/' $HOME/.empyrean/config/config.toml
sed -i'' 's/trust_height = 0/trust_height = <BLOCK_HEIGHT>/' $HOME/.empyrean/config/config.toml
sed -i'' 's/trust_hash = ""/trust_hash = "<BLOCK_HASH>"/' $HOME/.empyrean/config/config.toml
sed -i'' 's/rpc_servers = ""/rpc_servers = "https:\/\/cosmos-rpc.polkachu.com:443,https:\/\/rpc-cosmoshub-ia.cosmosia.notional.ventures:443,https:\/\/rpc.cosmos.network:443"/' $HOME/.empyrean/config/config.toml
...
> **Note**: Make sure to set the `--home` flag when initializing and starting `empyreand` if mounting quicksync data externally.

#### Create empyrean Home & Config

```bash
mkdir $HOME/.empyrean/config -p
```
...
```bash
curl https://quicksync.io/addrbook.cosmos.json > $HOME/.empyrean/config/addrbook.json
```
