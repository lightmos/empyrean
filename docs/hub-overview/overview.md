...
::: warning
### **v10 Upgrade**
Cosmos Hub will be upgraded to [v10](https://github.com/cosmos/empyrean/releases/tag/v10.0.0) at block height: **15,816,200**
...
`empyreand` is a command-line interface that lets you interact with the Cosmos Hub. `empyreand` is the only tool that supports 100% of the Cosmos Hub features, including accounts, transfers, delegation, and governance. Learn more about `empyreand` with the [delegator's CLI guide](../delegators/delegator-guide-cli.md).

## Running a full-node on the Cosmos Hub Mainnet

In order to run a full-node for the Cosmos Hub mainnet, you must first [install `empyreand`](../getting-started/installation.md). Then, follow [the guide](../hub-tutorials/join-mainnet.md).
