...
- /cmd/empyreand: Main applications for this project.
    - cmd/
    - main.go
...
git checkout v8.0.0
make build 
cp ./build/empyreand ./build/empyreand8
```

#### Build empyrean v9.0.0 and move into ./build:
```sh
git checkout v9.0.0
make build 
cp ./build/empyreand ./build/empyreand9
```
...
```sh
./contrib/scripts/run-empyrean-v8.sh
```

#### Run the upgrade
In another terminal window, run the script that waits 10 seconds for empyrean to start then makes gov proposal to perform an upgrade at height 15
```sh
...
- To distinguish SDK and empyrean with the same package name, add SDK or empyrean or the module name as the prefix. E.g.: `sdk/types`, `empyrean/types` and `empyrean/x/globalfee/types`, can use `sdktype`, `empyreantype`, `globalfeetype`.
